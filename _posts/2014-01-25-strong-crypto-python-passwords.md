---
layout: post
title: Strong crypto Python passwords
description: >
...
These indicators of underlying mechanisms and values are called *oracles*. Once one has an oracle that indicates the algorithm, one can quickly use a [dictionary attack](http://en.wikipedia.org/wiki/Dictionary_attack) to check for the presence of many passwords.

The KDF makes the oracle much more expensive. In addition to increasing the
number of comparisons necssary – one for every potential value of the iterations
– one can vary the number of iterations, so even where one matches a password to a secret that match generally reveals the secret in that one instance. With a regular hash or where the KDF iterations are constant the match will reveal the number of iterations.

Here is how KDF works, with just an 8 byte key for illustration, and a salt of "" (two double-quote characters):
...
You can [try it online](http://anandam.name/pbkdf2/). Note how the number of iterations significantly increases the time to produce the result.

Thus, even if one obtains the stored secrets one cannot as easily obtain the passwords used to authenticate individuals.
...
class Credentials(ndb.Model):
    ...
    """
    # our random stream - used for generating random bits for the salt
    # and for iterations entropy
    _randf = None
    ...
    # kength of the stored key
    DK_LEN = 32

    # This is a derived key from e.g. PBKDF2 (or, future: scrypt)
    dk = ndb.BlobProperty(indexed=False)

    # the salt is randomly generated for each dk
    salt = ndb.BlobProperty(indexed=False)

    # each dk has a random number of iterations, above ITERATIONS_2013
    # and -- in theory -- increasing in amount over time
...
We do not want our logs or any other conversion of the credentials to leak the contents of the model, so we overload the following to prevent that from happening.
...
We need to get some random data throughout the process, and this is how I achieve that.
...
`ITER_OFFSET` – very, very slightly increasing our resilience.
...
The weakness I perceive to the above is that the salt and the iterations (plus or minus some constant in the code) are both stored with the derived key. If the model is slurped then the number of iterations is apparent.

This is just a small piece of a security puzzle, and there are many other vectors one must bear in mind. This does raise the bar though for the divulging of the secrets stored to the disclosue of the authentication they represent.

For that reason, and to increase the discussion and awareness of the process of achieving crypto security, that I share the above.
